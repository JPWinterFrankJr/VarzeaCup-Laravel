...
class Time extends Model
{
    ...
    '), DB::raw('
        SUM(CASE 
            WHEN (vc_partidas.time1_id = vc_times.id AND (vc_partidas.time1_gols1 + vc_partidas.time1_gols2 > vc_partidas.time2_gols1 + vc_partidas.time2_gols2 )) OR
                 (vc_partidas.time2_id = vc_times.id AND (vc_partidas.time2_gols1 + vc_partidas.time1_gols1 > vc_partidas.time2_gols2 + vc_partidas.time1_gols2 ))
                  THEN 1
            ELSE 0 
        END) AS vitorias
    '), DB::raw('
        SUM(CASE 
            WHEN (vc_partidas.time1_id = vc_times.id AND (vc_partidas.time1_gols1 + vc_partidas.time1_gols2 < vc_partidas.time2_gols1 + vc_partidas.time2_gols2 )) OR
                 (vc_partidas.time2_id = vc_times.id AND (vc_partidas.time2_gols1 + vc_partidas.time1_gols1 < vc_partidas.time2_gols2 + vc_partidas.time1_gols2 )) THEN 1
            ELSE 0 
        END) AS derrotas
    '), DB::raw('
        SUM(CASE 
            WHEN (vc_partidas.time1_id = vc_times.id AND ((vc_partidas.time2_gols1 + vc_partidas.time1_gols1 = vc_partidas.time2_gols2 + vc_partidas.time1_gols2))) OR
                 (vc_partidas.time2_id = vc_times.id AND ((vc_partidas.time2_gols1 + vc_partidas.time1_gols1 = vc_partidas.time2_gols2 + vc_partidas.time1_gols2))) THEN 1
            ELSE 0 
        END) AS empates
    '))
